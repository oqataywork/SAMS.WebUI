...
[Attributes.IsAuthenticated(Order = 0), Attributes.AuthorizeUserType(Order = 1)]
	public partial class AssetsController : BaseController
	{
        ...
        [AccessLevelFilter(Action = AccessLevelEnum.Read, TableName = "tbl_Assets")]
        public async Task<ActionResult> Assets_Read([DataSourceRequest] DataSourceRequest request)
        {
            try
            {
                IEnumerable<AssetModel> cats =await AssetsService.ReadAssets();
        ...
        public AssetModel GetAssetByID(Guid id )
        {
            AssetModel mod = null;
            try
            {
               mod = AssetsService.OneAsset(id);
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
        ...
        public ActionResult GetAssetReadForm( string id)
        {

            try
            {
                AssetModel mod = GetAssetByID(new Guid(id));
                if(mod!=null)
                {
                    ViewBag.AssetID = mod.AssetID;
                    return PartialView("AssetReadFormPartial", mod);
                }
        ...
        public ActionResult GetAssetEditForm(string id)
        {
            ...
            try
            {
                ...
                {
                    ViewBag.AssetID = mod.AssetID;
                    return PartialView("AssetEditFormPartial", mod);
                }
                return Json(null, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
        ...
        public ActionResult GetAssetNewForm()
        {

            try
            {
                AssetModel mod = new AssetModel();
                if (mod != null)
                {
                    ViewBag.AssetID = mod.AssetID;
                    return PartialView("AssetEditFormPartial", mod);
        ...
        public async Task<ActionResult> Assets_Create([DataSourceRequest] DataSourceRequest request, AssetModel mod)
        {
            string _err = "";
            try
            {
        ...
        public async Task<ActionResult> Assets_Update([DataSourceRequest] DataSourceRequest request, AssetModel mod)
        {
            try
            {
                string _err = "";
                ...
                if (mod != null)
                {
                    ...
                    if (ModelState.IsValid)
                    {
                        ObjectResponse resp = await AssetsService.UpdateAsset(mod);
                        if (resp.Response != null)
                        {
                            _updated = (AssetModel)resp.Response;
                            mod = _updated;
                        }
                        else
                            ModelState.AddModelError("exception", resp.error);
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        [AccessLevelFilter(Action = AccessLevelEnum.Edit, TableName = "tbl_Assets")]
        public async Task<ActionResult> Assets_Create_or_Update([DataSourceRequest] DataSourceRequest request, AssetModel mod)
        {
            try
            {
                string _err = "";
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        [AccessLevelFilter(Action = AccessLevelEnum.Delete, TableName = "tbl_Assets")]
        public async Task<ActionResult> Assets_Destroy([DataSourceRequest] DataSourceRequest request, AssetModel mod)
        {
            try
            {
                string _err = "";
                
                ObjectResponse resp = await AssetsService.RemoveAsset(mod.AssetID);
